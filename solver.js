...
export function solveBlockDiagram(nodes, connections) {
    ...
    if (hasSymbolic) {
        ...
        return solveNumerically(nodes, connections);
    }
}
...
export function solveSymbolically(nodes, connections) {
        ...
        activeNodes.forEach((otherNode, j) => {
            if (!C[i][j].isZero()) {
                ...
                if (coeff.isOne()) {
                    ...
                    terms.push(`(${coeff.toKaTeX()}) \\cdot ${otherNode.label}`);
                }
            }
        });
    ...
    const steps = ["Identified symbolic letters inside block values.", "Formulating symbolic node equations..."];
    const outIdx = indexMap[outputNode.id];

    // Eliminating intermediate nodes step by step using substitution
    for (let k = 0; k < K; k++) {
        if (k === outIdx) continue;

        steps.push(`Eliminating intermediate variable ${activeNodes[k].label}...`);
    ...
    const finalSelfLoop = C[outIdx][outIdx];
    let rawFinalTF = V[outIdx];
    if (!finalSelfLoop.isZero()) {
        rawFinalTF = rawFinalTF.divide(SymExpr.const("1").subtract(finalSelfLoop));
    }

    steps.push(`Resolved final system equations for Output variable ${outputNode.label}.`);
